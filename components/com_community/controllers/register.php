	...
	public function registerUpdateProfile()
	{
		...
		foreach( $profiles as $key => $groups )
		{
			foreach( $groups->fields as $data )
			{
				...
				if( !CProfileLibrary::validateField( $data->type , $values[ $data->id ]->value , $data->required ) )
				{
        ...
        //redirect to avatar upload page. 
        $mainframe->redirect(CRoute::_('http://www.singmeout.com/throngbook/index.php/user-area.html' . $profileType, false));
	}
