...
class CommunityModelRegister extends JCCModel
{
	...
	public function cleanTempUser(){
		...
		$query = 'DELETE FROM '.$db->nameQuote('#__community_register');
		$query .= ' WHERE '.$db->nameQuote('created').' <= DATE_SUB('.$db->Quote($nowDateMysql).',  INTERVAL 10 MINUTE)';		
		...
		$query .= ' WHERE '.$db->nameQuote('created').' <= DATE_SUB('.$db->Quote($nowDateMysql).',  INTERVAL 5 MINUTE)';
