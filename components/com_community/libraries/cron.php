...
class CCron {
	...
	private function _processAvatarStorage( $updateNum = COMMUNITY_PROCESS_STORAGE_LIMIT , $element = 'users' )
	{
		...
		$this->message[] = JText::_('Moving ' . $element . ' avatars to proper storage location');
		...
		foreach( $rows as $row )
		{
			...
			if( $current->exists($row->thumb) && $current->exists($row->avatar) )
			{
				...
				$this->message[] = JText::sprintf('Downloading %1$s thumbnail to temporary location: %2$s' , $element , $tmpThumbFileName );
				$current->get( $row->thumb , $tmpThumbFileName );
				
				$tmpAvatarFileName	= $jconfig->getValue( 'tmp_path' ) . DS . md5( $row->avatar );
				$this->message[] = JText::sprintf('Downloading %1$s avatar to temporary location: %2$s' , $element , $tmpAvatarFileName );
	...
	{
		$this->message[] =  'Remove deleted files';
		$db=& JFactory::getDBO();
	...
	public function _processPhotoStorage($updateNum = 5){
		...
		{
			$this->message[] = 'Moving files to remote storage.';
			...
			foreach($result as $row) 
			{
				...
				if( $row->storage == 'file' && !JFile::exists(JPATH_ROOT.DS.$row->image) )
				{
					...
					$this->message[] = 'Image file missing. Creating image file from '.$row->original;
				...
				if( $currentStorage->exists($row->image) && $currentStorage->exists($row->thumbnail) )
				{
					...
					$this->message[] = 'Downloading image to temporary location: '.$tempFilename;
					$currentStorage->get($row->image, $tempFilename);
					
					$thumbsTemp		= $jconfig->getValue('tmp_path'). DS . 'thumb_' . md5($row->thumbnail);
					$this->message[] = 'Downloading thumbnail to temporary location: '.$thumbsTemp;
	...
	public function _processVideoStorage($updateNum=5)
	{
		...
		$this->message[] = 'Moving videos to the preferred storage: ' . $videoStorage ;
		...
		foreach ($result as $videoEntry)
		{
			...
			if ($videoEntry->type == 'file')
			{
				...
				if ($currentStorage->exists($videoEntry->path))
				{
					...
					$this->message[] = 'Downloading video to temporary location: '.$tempFilename;
					$currentStorage->get($videoEntry->path, $tempFilename);
					$this->message[] = 'Downloading video to temporary location: '.$tempThumbname;
	...
	public function _sendSiteDetails()
	{
		...
		$this->message[] = "Updating to Jomsocial Network";
	}
	...
	public function _sendEmails()
	{
		...
		$this->message[] = JText::_('COM_COMMUNITY_SENDING_EMAILS');
	}
	...
	{
		$this->message[] = 'Marking temporary photos for deletion';
	...
	{
		$this->message[] = 'Deleting temporary videos.';
