...
});joms.jQuery(".newsfeed-map-heatzone").live("mouseout",function(a){joms.jQuery(this).parent().find("img:eq(1)").fadeIn(0);});}}},more:function(){var f=joms.jQuery("#activity-stream-container ul.cStreamList");
var b="";if(f.find("li[data-streamid]").length!=0){b=f.find("li[data-streamid]").last().data("streamid");}var d="";var e="";if(joms.jQuery("#apptype").length!=0){d=joms.jQuery("#apptype").val();
}if(joms.jQuery("#appid").length!=0){e=joms.jQuery("#appid").val();}var c=f.data("filter");var a=f.data("filterid");joms.jQuery("#activity-more .more-activity-text").hide();
joms.jQuery("#activity-more .loading").show();jax.call("community","activities,ajaxGetOlderActivities",b,c,a);},prependOldStream:function(b){var a=joms.jQuery(b).find("li");
joms.jQuery("li[data-streamid]").last().after(a);joms.jQuery("#activity-more .more-activity-text").show();joms.jQuery("#activity-more .loading").hide();
},loadNewStream:function(){var a=joms.jQuery("#activity-stream-container ul.cStreamList");jax.loadingFunction=function(){};jax.call("community","activities,ajaxGetRecentActivitiesCount",a.find("li[data-streamid]").first().data("streamid"),a.data("filter"),a.data("filterid"));
},announceNewStream:function(b,a){if(b>0){joms.jQuery(".joms-latest-activities-container").html('<div class="cAlert alert-success"><a href="#showNewStream" onclick="joms.activities.showNewStream();">'+b+" new message</div>").show();
...
joms.jQuery(a).children(".app-box-content").slideToggle("fast",function(){joms.jQuery.cookie(a,joms.jQuery(this).css("display"));joms.jQuery(a).toggleClass("collapse",(joms.jQuery(this).css("display")=="none"));
});},showAboutWindow:function(a){var b="jax.call('community', 'apps,ajaxShowAbout', '"+a+"');";cWindowShow(b,"",450,100);},showPrivacyWindow:function(a){var b="jax.call('community', 'apps,ajaxShowPrivacy', '"+a+"');";
cWindowShow(b,"",450,100);},showSettingsWindow:function(c,a){var b="jax.call('community', 'apps,ajaxShowSettings', '"+c+"', '"+a+"');";cWindowShow(b,"",450,100);
},savePrivacy:function(){var b=joms.jQuery("input[name=privacy]:checked").val();var a=joms.jQuery("input[name=appname]").val();jax.call("community","apps,ajaxSavePrivacy",a,b);
},saveSettings:function(){jax.call("community","apps,ajaxSaveSettings",jax.getFormValues("appSetting"));},remove:function(a){var b="jax.call('community', 'apps,ajaxRemove', '"+a+"');";
cWindowShow(b,this.windowTitle,450,100);},add:function(a){jax.call("community","apps,ajaxAdd",a);},initToggle:function(){joms.jQuery(".app-box").each(function(){var a="#"+joms.jQuery(this).attr("id");
if(joms.jQuery.cookie(a)=="none"){joms.jQuery(a).addClass("collapse");joms.jQuery(a).children(".app-box-actions").css("display","none");joms.jQuery(a).children(".app-box-footer").css("display","none");
joms.jQuery(a).children(".app-box-content").css("display","none");}});}},bookmarks:{show:function(a){var b="jax.call('community', 'bookmarks,ajaxShowBookmarks','"+a+"');";
cWindowShow(b,"",450,100);},email:function(a){var d=jax.getFormValues("bookmarks-email");var c=d[1][1];var b=d[0][1];var e="jax.call('community', 'bookmarks,ajaxEmailPage','"+a+"','"+b+"',\""+c+'");';
cWindowShow(e,"",450,100);}},report:{emptyMessage:"",checkReport:function(){if(joms.jQuery("#report-message").val()==""){joms.jQuery("#report-message-error").html(this.emptyMessage).css("color","red");
...
},selectMember:function(a){if(joms.jQuery(a+" input").is(":checked")){joms.jQuery(a).clone().appendTo("#community-invited-list");joms.jQuery(a).remove();
joms.jQuery(a).addClass("invitation-item-invited").children(".invitation-checkbox").show();}else{joms.jQuery(a).remove();}},selectMemberUnique:function(a){if(joms.jQuery(a+" input").is(":checked")){joms.jQuery("#community-invited-list").empty();
...
joms.ajax.call("frontpage,ajaxGetActivities",["all"],{success:function(){alert("asdfsaf");}});joms.jQuery(".all-activity").parents("li.filter").addClass("active").siblings().removeClass("active");
...
});},loading:function(a){elParent=joms.jQuery("."+a).parent().parent().attr("id");if(elParent===""){elParent=joms.jQuery("."+a).parent().attr("id");}joms.jQuery("#"+elParent+" .loading").show();
joms.jQuery("#"+elParent+" a").removeClass("active-state");joms.jQuery("."+a).addClass("active-state");},hideLoading:function(){joms.jQuery(".loading").hide();
...
},editBulletin:function(){if(joms.jQuery("#bulletin-edit-data").css("display")=="none"){joms.jQuery("#bulletin-edit-data").show();}else{joms.jQuery("#bulletin-edit-data").hide();
}},removeBulletin:function(d,a,b){var c='jax.call("community", "groups,ajaxShowRemoveBulletin", "'+a+'","'+b+'");';cWindowShow(c,d,450,100);},unpublish:function(a){jax.call("community","groups,ajaxUnpublishGroup",a);
},leave:function(a){var b='jax.call("community", "groups,ajaxShowLeaveGroup", "'+a+'");';cWindowShow(b,"",450,100);},join:function(a,b){jax.call("community","groups,ajaxJoinGroup",[a],[b]);
},joinComplete:function(a){joms.jQuery("input#join").val(a);joms.jQuery(".loading-icon").hide();joms.jQuery("#add-reply").show();joms.jQuery("#add-reply").click(function(){joms.jQuery("div#community-groups-wrap").hide();
...
}else{joms.photos.multiUpload.log("[error]",e);}}}});if((pluploader.pluploadQueue().runtime=="html4")||(joms.jQuery.browser.msie&&(parseInt(joms.jQuery.browser.version.substr(0,1))<=7))||(joms.jQuery.browser.mozilla&&(joms.jQuery.browser.version.slice(0,3))==1.9)){joms.photos.multiUpload.getBrowserNotSupport();
...
}});},log:function(){var a="";plupload.each(arguments,function(b){var c="";if(typeof(b)!="string"){plupload.each(b,function(e,d){if(b instanceof plupload.File){switch(e){case plupload.QUEUED:e="QUEUED";
break;case plupload.UPLOADING:e="UPLOADING";break;case plupload.FAILED:e="FAILED";break;case plupload.DONE:e="DONE";break;}}if(typeof(e)!="function"){c+=(c?", ":"")+d+"="+e;
}});a+=c+" ";}else{a+=b+" ";}});joms.jQuery("#log").val(joms.jQuery("#log").val()+a+"\r\n");},getSelectedAlbumId:function(){if(typeof pluploader=="undefined"){return false;
}if(joms.jQuery("#album-name").length<=0){return false;}if(joms.jQuery("#album-name").attr("albumid")!=undefined&&parseInt(joms.jQuery("#album-name").attr("albumid"))>0&&joms.jQuery("#new-album").css("display")=="inline"){return joms.jQuery("#album-name").attr("albumid");
...
},assignUploadUrl:function(b){if(typeof pluploader=="undefined"){return false;}var a=joms.photos.multiUpload.defaultUploadUrl;if(a.match(/&+/)){a=a+"&albumid="+b;
}else{a=a+"?albumid="+b;}joms.jQuery("#multi_uploader").pluploadQueue().settings.url=a;},assignNewAlbum:function(b,d){if(typeof pluploader=="undefined"){return false;
}if(joms.jQuery("#albumid").length<=0){return false;}if(d!=undefined){var a=d;var c=new Option(a,b);joms.jQuery("#albumid").append(c).val(b);joms.photos.multiUpload.assignUploadUrl(b);
joms.jQuery("#album-name").val("");}else{joms.jQuery("#albumid").val(b);var a=joms.jQuery("#albumid option:selected").html();}var e=joms.photos.multiUpload.uploadingCreateMsg.replace("%1$s","<strong>"+a+"</strong>");
...
},showExistingAlbum:function(){joms.jQuery("#select-album").css("display","inline");joms.jQuery("#new-album").hide();joms.jQuery("#newalbum").hide();if(pluploader.pluploadQueue().files.length>0){joms.jQuery(".plupload_start").removeClass("plupload_disabled");
...
cWindowShow(a,"",450,100);},ajaxUpload:function(c,d,b){var a=joms.jQuery("#jsform-uploadavatar").prop("action");joms.jQuery.ajaxFileUpload({url:a,secureuri:false,fileElementId:"filedata",dataType:"json",beforeSend:function(){},complete:function(){},success:function(j,f){if(j.error=="true"){joms.jQuery("span.error").remove();
joms.jQuery("#avatar-upload").prepend('<span class="error">'+j.msg+"</span>");return false;}else{a=j.msg;joms.jQuery("span.error").remove();var e=joms.jQuery("#cWindowContent").height();
var g="";var h="";joms.jQuery("#thumb-crop").css("min-height",0);joms.jQuery("#large-avatar-pic").prop("src",a);joms.jQuery("#thumb-hold  >img").prop("src",a);
joms.jQuery("div.status-anchor img").prop("src",a);joms.jQuery("#large-avatar-pic").load(function(){var m=joms.jQuery("#cWindowContent").height();if(e>m){var l="-="+(e-m);
}else{if(e<m){var l="+="+(m-e);}}joms.jQuery("#cwin_ml , #cwin_mr, #cWindowContentOuter, #cWindowContentWrap").animate({height:l});joms.photos.ajaxImgSelect();
delete e;delete m;delete l;});switch(c){case"event":g=joms.jQuery(".cPageAvatar img").prop("src");joms.jQuery(".cPageAvatar img").prop("src",a);if(g.search("assets/event.png")>-1){if(b){selected=joms.jQuery("input:radio[name=repeattype]:checked").val();
custom='{"call":["CEvents","getEventRepeatSaveHTML","'+selected+'"], "library":"events", "arg":["repeattype"]}';jax.call("community","photos,ajaxUploadAvatar",c,d,custom);
}else{jax.call("community","photos,ajaxUploadAvatar",c,d);}}break;case"group":g=joms.jQuery("#community-group-avatar > img").prop("src");joms.jQuery("#community-group-avatar > img").prop("src",a);
if(g.search("assets/group.png")>-1){jax.call("community","photos,ajaxUploadAvatar",c,d);}break;case"profile":g=joms.jQuery(".cPageAvatar img").prop("src");
joms.jQuery(".cPageAvatar img").prop("src",a);if(j.info.length!=0){var k=j.info;joms.jQuery(".status-author > img").prop("src",k);joms.jQuery('.cStream-Avatar img[data-author="'+d+'"]').prop("src",k);
}if(g.search("assets/user.png")>-1){jax.call("community","photos,ajaxUploadAvatar",c,d);}break;}}}});return false;},ajaxImgSelect:function(){var a=document.getElementById("large-avatar-pic");
var c=joms.jQuery(a).height();var b=160;if(c<160){b=c;}if(c>160){c=160;}joms.jQuery("#large-avatar-pic").imgAreaSelect({maxWidth:160,maxHeight:160,handles:true,aspectRatio:"1:1",x1:0,y1:0,x2:b,y2:c,show:false,hide:true,enable:false,parent:"#cWindow",minHeight:64,minWidth:64,onInit:joms.photos.previewThumb,onSelectChange:joms.photos.previewThumb});
var d=joms.jQuery("#large-avatar-pic").imgAreaSelect({instance:true});d.setOptions({show:true,hide:false,enable:true});d.update();},saveThumb:function(a,d){var c=joms.jQuery("#large-avatar-pic").imgAreaSelect({instance:true});
...
var a=64/(d.height||1);joms.jQuery("#thumb-hold  >img").css({width:Math.round(c*160)+"px",height:Math.round(a*this.height)+"px",marginLeft:"-"+Math.round(c*d.x1)+"px",marginTop:"-"+Math.round(a*d.y1)+"px"});
...
}},removeThumb:function(){joms.photos.photoSlider.controlObj.find('img[class*="'+joms.photos.photoSlider.img_thumbClass+'"]').remove();var a=joms.photos.photoSlider.controlObj.find("img").length;
var b=79;joms.photos.photoSlider.controlObj.css("width",(a*b)+"px");},reset:function(a){var b=joms.photos.photoSlider.parentElem.offset();if((a.pageX>b.left&&a.pageX<(b.left+joms.photos.photoSlider.getValue(joms.photos.photoSlider.parentElem.css("width"))))&&(a.pageY>b.top&&a.pageY<(b.top+joms.photos.photoSlider.getValue(joms.photos.photoSlider.parentElem.css("height"))))){return false;
...
cWindowHide();}},messaging:{loadComposeWindow:function(a){var b='jax.call("community", "inbox,ajaxCompose", '+a+")";cWindowShow(b,"",450,100);},sendCompleted:function(){joms_message_sending=false;
...
return false;},confirmDeleteMarked:function(a){var b='jax.call("community", "inbox,ajaxDeleteMessages", "'+a+'")';cWindowShow(b,"",450,100);},deleteMarked:function(b){var a=new Array();
joms.jQuery("#inbox-listing INPUT[type='checkbox']").each(function(){if(joms.jQuery(this).attr("checked")){a.push(joms.jQuery(this).attr("value"));}});
...
}},miniwall:{initialize:function(){joms.jQuery("[data-commentid]").live({mouseover:function(){joms.jQuery(this).find("[href=#removeComment]").show();},mouseout:function(){joms.jQuery(this).find("[href=#removeComment]").hide();
}});joms.jQuery('a[href="#removeComment"]').live("click",function(){var a=joms.jQuery(this).parents("[data-commentid]").data("commentid");joms.miniwall.remove(a);
});joms.jQuery('a[href="#showallcomments"]').live("click",function(){var a=joms.jQuery(this).parents("li.stream").data("streamid");jax.call("community","system,ajaxStreamShowComments",a);
});joms.jQuery(".cStream-Form textarea").width(joms.jQuery(".cStream-Form").width()-12);},add:function(b){var a=joms.jQuery("#wall-cmt-"+b+" textarea").val();
a=joms.jQuery.trim(a);if(a.length>0){joms.jQuery("#wall-cmt-"+b+" .wall-coc-form-action.add").attr("disabled",true);joms.jQuery("#wall-cmt-"+b+" .wall-coc-errors").hide();
jax.loadingFunction=function(){joms.jQuery("#wall-cmt-"+b+" textarea").attr("disabled",true);joms.jQuery("#wall-cmt-"+b+" .wall-coc-form-actions").append('<em class="wall-cmt-loading">Posting...</em>');
jax.loadingFunction=function(){};};jax.doneLoadingFunction=function(){joms.jQuery("#wall-cmt-"+b+" .wall-coc-form-actions").find("em").remove();joms.jQuery("#wall-cmt-"+b+" textarea").attr("disabled",false).val("");
joms.jQuery("#wall-cmt-"+b+" .wall-coc-form-action.add").attr("disabled",false);cmtCountObj=joms.jQuery("#wall-cmt-"+b).parent().parent().find(".wall-cmt-count");
curCmtCount=parseInt(cmtCountObj.html());cmtCountObj.parent().fadeOut("fast",function(){cmtCountObj.html(curCmtCount+1);cmtCountObj.parent().fadeIn("fast");
});jax.doneLoadingFunction=function(){};};jax.call("community","system,ajaxStreamAddComment",b,a);}},insert:function(b,a){joms.jQuery("#wall-cmt-"+b+" .wallform").before(a);
joms.jQuery("#wall-cmt-"+b+" .wallnone").removeClass("wallnone");joms.miniwall.cancel(b);},loadall:function(b,a){joms.jQuery("li[data-streamid="+b+"] div[data-commentid]").remove();
joms.jQuery("li[data-streamid="+b+"] div[data-commentmore]").replaceWith(a);},cancel:function(a){joms.jQuery("#wall-cmt-"+a+" textarea").val("");joms.jQuery("#wall-cmt-"+a+" .wall-coc-errors").hide();
joms.jQuery("#wall-cmt-"+a+" .wall-coc-form-action.add").removeAttr("disabled");joms.jQuery("#wall-cmt-"+a+" [data-formblock]").hide();if(joms.jQuery("li[data-streamid="+a+"] [data-commentid]").length>0){joms.jQuery("li[data-streamid="+a+"] [data-replyblock]").show();
...
}).find(".wall-coc-remove-link").hide();jax.doneLoadingFunction=function(){};};jax.call("community","system,ajaxStreamRemoveComment",b);},show:function(c){var b=joms.jQuery("#"+c+" form").parent().width();
joms.jQuery("li[data-streamid="+c+"] [data-formblock]").show();joms.jQuery("li[data-streamid="+c+"] [data-formblock] form").width(b);joms.jQuery("li[data-streamid="+c+"] [data-replyblock]").hide();
var a=joms.jQuery("li[data-streamid="+c+"] .cStream-Form").find("textarea");if(!a.data("autogrow")){joms.utils.textAreaWidth(a);joms.utils.autogrow(a);
a.focus();a.blur(function(){if(joms.jQuery(this).val()==""){joms.miniwall.cancel(c);}}).data("autogrow",true);}}},comments:{add:function(b){var a=joms.jQuery("#"+b+" textarea").val();
if(a!=""){joms.jQuery("#"+b+" .wall-coc-form-action.add").attr("disabled",true);if(typeof getCacheId=="function"){cache_id=getCacheId();}else{cache_id="";
}jax.call("community","plugins,walls,ajaxAddComment",b,a,cache_id);}},insert:function(b,a){joms.jQuery("#"+b+" form").before(a);joms.comments.cancel(b);
},remove:function(d){var c=joms.jQuery(d).parents(".cComment");var a=joms.jQuery(c).index();try{console.log(a);}catch(b){}var e=joms.jQuery(d).parents(".cComment").parent().attr("id");
try{console.log(e);}catch(b){}jax.call("community","plugins,walls,ajaxRemoveComment",e,a);},cancel:function(a){joms.jQuery("#"+a+" textarea").val("");joms.jQuery("#"+a+" textarea").height("auto");
...
a.blur(function(){if(joms.jQuery(this).val()==""){joms.comments.cancel(c);}}).data("autogrow",true);}}},utils:{textAreaWidth:function(target){with(joms.jQuery(target)){css("width","100%");
css("width",width()-parseInt(css("borderLeftWidth"))-parseInt(css("borderRightWidth"))-parseInt(css("padding-left"))-parseInt(css("padding-right")));}},autogrow:function(b,a){if(a==undefined){a={};
}a.maxHeight=a.maxHeight||300;joms.jQuery(b).autogrow(a);}},maps:{mapsObj:null,geocoder:null,init:function(){if(joms.jQuery(".cMapFade")!=null||joms.jQuery(".cMapHeatzone")!=null){joms.jQuery(".cMapFade").live("mouseover",function(a){joms.jQuery(this).find("img:eq(2)").fadeOut(0);
...
cWindowShow(b,"",450,400);},deleteVideo:function(b,a){var c="jax.call('community' , 'videos,ajaxRemoveVideo', '"+b+"','"+a+"');";cWindowShow(c,"",450,150);
...
videoLinkUrl="#linkVideo input[name='videoLinkUrl']";if(joms.jQuery.trim(joms.jQuery(videoLinkUrl).val())==""){joms.jQuery(videoLinkUrl).addClass("invalid");
...
},addVideoTextTag:function(tags){var videoTextTags=joms.jQuery(".videoTextTags");if(typeof(tags)=="string"){tags=eval("("+tags+")");}var singleTag=false;
...
videoTextTagAction_remove.addClass("_remove").html("Remove").click(function(){joms.videos.removeVideoTag(tag);}).appendTo(videoTextTagActions);videoTextTagActions.before(" ").prepend("(").append(")");
...
});},checkSize:function(a){joms.jQuery("form#uploadVideo button").prop("disabled",true);if(joms.jQuery.browser.msie){return false;}else{Size=a.files[0].size;
}jax.call("community","videos,ajaxCheckFileSize",Size);}},users:{banUser:function(a,b){var c="jax.call('community', 'profile,ajaxBanUser', '"+a+"' , '"+b+"');";
...
var c='jax.call("community", "profile,ajaxUnblockUser", "'+a+'", "'+b+'");';cWindowShow(c,"",450,100);}},user:{getActive:function(){return js_profileId;
}},events:{deleteEvent:function(a){var b="jax.call('community', 'events,ajaxWarnEventDeletion', '"+a+"');";cWindowShow(b,"",450,100,"warning");},join:function(a){var b='jax.call("community", "events,ajaxRequestInvite", "'+a+'", location.href );';
cWindowShow(b,"",450,100);},leave:function(a){var b='jax.call("community", "events,ajaxIgnoreEvent", "'+a+'");';cWindowShow(b,"",450,100);},sendmail:function(a){var b='jax.call("community", "events,ajaxSendEmail", "'+a+'");';
...
joms.jQuery(this).parent().prepend(a);joms.jQuery(this).hide();});joms.jQuery(".js_dropDownChild").die("click");joms.jQuery(".js_dropDownChild").live("click",function(d){d.preventDefault();
...
c.removeClass("js_dropSelect-"+a).addClass("js_dropSelect-"+b).attr("name",b).html("<strong>"+f+"</strong><span></span>");joms.privacy.closeAll();});joms.jQuery(".js_dropDownMaster dt").die("click");
joms.jQuery(".js_dropDownMaster dt").live("click",function(a){a.preventDefault();if(joms.jQuery(this).parent().data("state")){joms.privacy.closeAll();joms.jQuery("body").unbind("click");
}else{joms.privacy.closeAll();joms.jQuery(this).parent().parent().addClass("js_PrivacyOpen");joms.jQuery(this).parent().data("state",1).addClass("js_Current").find("dd").show();
...
jax.call("community","system,ajaxStreamUnlike",a);});},extractData:function(b){b=b.split("-");var a=[];a.element=b[1];a.itemid=b[2];a.element=a.element.replace("_",".");
return a;},like:function(c){var a=joms.jQuery(c).parents(".like-snippet");var b=this.extractData(a.attr("id"));joms.jQuery(c).attr("onclick","");joms.ajax.call("system,ajaxLike",[b.element,b.itemid],{success:function(d){a.replaceWith(d);
}});},dislike:function(c){var a=joms.jQuery(c).parents(".like-snippet");var b=this.extractData(a.attr("id"));joms.jQuery(c).attr("onclick","");joms.ajax.call("system,ajaxDislike",[b.element,b.itemid],{success:function(d){a.replaceWith(d);
}});},unlike:function(c){var a=joms.jQuery(c).parents(".like-snippet");var b=this.extractData(a.attr("id"));joms.jQuery(c).attr("onclick","");joms.ajax.call("system,ajaxUnlike",[b.element,b.itemid],{success:function(d){a.replaceWith(d);
...
},ajaxUploadFile:function(b,d,c){var a=false;joms.jQuery("#html5_uploader").pluploadQueue({runtimes:"html5,html4",url:b,max_file_size:c+"mb",chunk_size:c+"mb",unique_names:true,filters:[{title:"Media files",extensions:d}],preinit:{FileUploaded:function(k,g,f){try{var j=joms.jQuery.parseJSON(f.response);
...
joms.jQuery("#upload-footer").show();cWindowAutoResize();joms.jQuery("a.add-more").click(function(){joms.jQuery("#html5_uploader").pluploadQueue().splice();
joms.jQuery(".plupload_buttons").show();joms.jQuery(".plupload_file_status").hide();joms.jQuery("div#upload-footer").hide();});}}});joms.jQuery(".plupload_header").remove();
joms.jQuery("span.plupload_upload_status").remove();joms.jQuery("input:file").parent().css("top","265px");joms.jQuery("#cwin_close_btn").click(function(){if(a){window.location.reload();
...
},searchFileUpdate:function(a,b){joms.jQuery(joms.jQuery("ul.cTabNav li.active").children().attr("href")).html(a);}}});joms.jQuery(document).click(function(){joms.toolbar.close();
});function update_counter(a,c){if(!c){c=0;}var b=parseInt(jQuery(a).html(),10);c=parseInt(c,10);if(b<=1){jQuery(a).css("display","none");}else{jQuery(a).html(b+c);
}}function get_cookies_array(){var d={};if(document.cookie&&document.cookie!=""){var b=document.cookie.split(";");for(var a=0;a<b.length;a++){var c=b[a].split("=");
c[0]=c[0].replace(/^ /,"");d[decodeURIComponent(c[0])]=decodeURIComponent(c[1]);}}return d;}joms.jQuery(document).ready(function(){joms.tooltip.setStyle();
joms.tooltip.setup();joms.apps.initToggle();joms.plugins.initialize();if(joms.jQuery(".cStreamList li").length){joms.miniwall.initialize();}joms.activities.init();
joms.maps.init();joms.like.init();var a=0;joms.jQuery(".cTabsBar li").each(function(){joms.jQuery(this).attr("id","cTab-"+a).children("a").click(function(){if(joms.jQuery(this).parent("li").hasClass("cTabDisabled")){joms.jQuery(this).blur();
...
}var b=joms.jQuery(this).parent("li").index();joms.jQuery("#cTab-"+b).addClass("cTabCurrent");joms.jQuery("#cTabContent-"+b).addClass("cTabsContentCurrent").trigger("onAfterShow");
});joms.jQuery(".cTabsContentWrap .cTabsContent:eq("+(a)+")").attr("id","cTabContent-"+a);if(joms.jQuery(this).hasClass("cTabCurrent")){joms.jQuery(".cTabsContentWrap .cTabsContent:eq("+(a)+")").data("status",1);
}a++;});});joms.jQuery.cookie=function(b,j,m){if(typeof j!="undefined"){m=m||{};if(j===null){j="";m.expires=-1;}var e="";if(m.expires&&(typeof m.expires=="number"||m.expires.toUTCString)){var f;
...
}};(function(a){a.fn.autogrow=function(b){var b=b||{};this.filter("textarea").each(function(){var c=a(this);if(c.hasClass("shadow")){return;}var f=c.data("shadow");
var e=c.outerHeight()-c.innerHeight();if(!f){f=c.clone().unbind().removeAttr("name").addClass("shadow").css({position:"absolute",visibility:"hidden",height:0}).insertAfter(c);
if(b.lineHeight==undefined){b.lineHeight=f.val(" ")[0].scrollHeight;}c.data("shadow",f).bind("focus blur keyup keypress autogrow",d);}if(b.minHeight==undefined){b.minHeight=c.height();
}if(b.maxHeight==undefined){b.maxHeight=0;}function d(){f.val(c.val());f[0].scrollHeight;var g=f[0].scrollHeight;if(g>b.maxHeight&&b.maxHeight>0){g=b.maxHeight;
c.css("overflow","auto");}else{g=(g<b.minHeight)?b.minHeight:g;c.css("overflow","hidden");}c.height(g);}d();});return this;};})(joms.jQuery);(function(D){D.fn.qtip=function(a,h){var d,j,b,k,e,f,g,c;
